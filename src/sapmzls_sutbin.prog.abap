*&---------------------------------------------------------------------*
*& Module Pool       ZMTD_SCAN_E0322_SUTBIN
*&
***********************************************************************
* PROGRAM DECLARATION
***********************************************************************
* PROGRAM ID:         ZMTD_SCAN_E0322_SUTBIN
* AUTHOR Name:        Anup Varghese
* OWNER(Process Team) Lakshmikumar Reddy
* CREATE DATE:        11/11/2016
* ECC RELEASE VERSION 6.0
* BASED-ON PROGRAM:   None
* Object ID:          E0322
* DESCRIPTION :       This SCAN development is required to move storage
*                     units between storage bins
***********************************************************************
* VERSION CONTROL (Most recent on top):
* DATE             AUTHOR       CTS REQ        DESCRIPTION
*----------------------------------------------------------------------
* 01/31/2017       Anup V       ED2K909553     INC4815892/INC480991
* 11/11/2016       Anup V       ED2K909260     Initial Version
***********************************************************************


INCLUDE SAPMZLS_SUTBIN_TOP.
*INCLUDE zmtd_scan_e0322_sutbin_top              .    " global Data

INCLUDE SAPMZLS_SUTBIN_PBO.
*INCLUDE zmtd_scan_e0322_sutbin_pbo.

INCLUDE SAPMZLS_SUTBIN_PAI.
*INCLUDE zmtd_scan_e0322_sutbin_pai.

INCLUDE SAPMZLS_SUTBIN_FORMS.
*INCLUDE zmtd_scan_e0322_sutbin_forms.
