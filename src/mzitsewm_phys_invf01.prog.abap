...
*-----check if at least one bin on doc is not cancelled, cleared.
  ...
  IF v_bin_on_doc EQ abap_false.
    ...
    RETURN.
  ELSE.
*---Begin of add by Vicky on 20150325 - Check whether the bin was scanned before
    ...
    IF sy-subrc EQ 0.
      ...
      RETURN.
    ENDIF.
*---End of add by Vicky on 20150325
...
*--Create Class Object for validation
  ...
  CALL METHOD lo_hu->hubarcode_value
    ...
    IMPORTING
      ev_hunumber = lv_barcode.
  zsits_scan_dynp-zzbarcode = lv_barcode .
  ...
  ELSE.
    RETURN.
  ENDIF.
*  Project One - Begin of insert Rvenugopal
*     Check authorization
  ...
  IF ls_return IS NOT INITIAL.
    ...
    RETURN.
  ENDIF.
*  End of inser rvenugopal
* End of add by Vicky on 20150327

* Begin of add by Vicky on 20150324 - add check about storage bin
...
*   Storage unit &1 is already in storage bin &2 &3.
    ...
    RETURN.
  ENDIF.
* End of add by Vicky on 20150324

* Begin of add by Vicky on 20150326 - add check about scanning duplicate SU
  lv_wminvdoc = zsits_scan_dynp-zzwminvdoc.
...
*   &1 already counted.
    ...
    RETURN.
  ENDIF.
* End of add by Vicky on 20150326
...
*     Read bar code, get label type and content
      CALL METHOD zcl_its_utility=>barcode_read
        EXPORTING
          iv_barcode          = zsits_scan_dynp-zzbchmtr
          iv_exist_check      = abap_false    "If No RM batch exist
          is_read_option      = lx_read_option
          it_label_type_range = lt_label_type
        IMPORTING
          ev_label_type       = lv_label_type
...
* Begin of add by Vicky on 20150326
...
* Begin of add by Vicky on 20150330
...
*         &1 has stock in multiple plants.
    ...
    ENDIF.
  ENDIF.
* End of add by Vicky on 20150330
...
*     Begin of add by Vicky on 20150331 - Mark all items of scanned bin as zero when click 'Empty Bin'
      PERFORM frm_bin_empty.
*     End of add by Vicky on 20150331
...
*     Begin of add by Vicky on 20150325 - Bin can not be scanned again after 'Bin complete'
...
* Comment by Vicky on 20150330 - Mark the count to be zero for empty bin
...
*----------------------------------------------------------------------
  ...
  PERFORM update_zlwminvdoc USING gt_zlwminvdoc
                                  zsits_scan_dynp .
